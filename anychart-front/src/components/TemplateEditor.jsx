...
// 数据源适配器列表
const DATA_SOURCES = [{id: 'mock-adapter', name: 'Mock', icon: '🎲', color: '#4caf50'}, {
    id: 'rest-adapter', name: 'REST', icon: '🌐', color: '#2196f3'
}, {id: 'database-adapter', name: '数据库', icon: '💾', color: '#ff9800'}];

// Mock 数据查询列表
const MOCK_QUERIES = [{id: 'mock:sales', name: '销售数据'}, {
    id: 'mock:trend', name: '趋势数据'
}, {id: 'mock:distribution', name: '分布数据'}, {id: 'mock:correlation', name: '相关性'}, {
    id: 'mock:growth', name: '增长数据'
}];

// JsonPath 常用模板
const JSONPATH_TEMPLATES = ['$.items[*].date', '$.items[*].value', '$.items[*]', '$.data', '$[*]'];

// 图表类型配置
const CHART_TYPES = [{
    id: 'bar', name: '柱状图', icon: '📊', defaultConfig: {
        title: '柱状图', xAxisName: '类别', yAxisName: '数值', showLegend: true, showTooltip: true
    }
}, {
    id: 'line', name: '折线图', icon: '📈', defaultConfig: {
        title: '折线图', xAxisName: '时间', yAxisName: '数值', showLegend: true, showTooltip: true, smooth: false
    }
}, {
    id: 'pie', name: '饼图', icon: '🥧', defaultConfig: {
        title: '饼图', showLegend: true, showTooltip: true, showLabel: true
    }
}, {
    id: 'scatter', name: '散点图', icon: '⚬', defaultConfig: {
        title: '散点图', xAxisName: 'X轴', yAxisName: 'Y轴', showLegend: true, showTooltip: true
    }
}, {
    id: 'area', name: '面积图', icon: '📉', defaultConfig: {
        title: '面积图', xAxisName: '时间', yAxisName: '数值', showLegend: true, showTooltip: true
    }
}];
...
function generateChartOption(chartType, config, bindings) {
    ...
    switch (chartType) {
        case 'bar':
            return {
                ...baseOption, xAxis: {
                    type: 'category', name: config.xAxisName, data: '{{binding:categories}}'
                }, yAxis: {
                    type: 'value', name: config.yAxisName
                }, series: [{
                    type: 'bar', data: '{{binding:data1}}', name: '系列1'
                }]
            };
        ...
        case 'line':
            return {
                ...baseOption, xAxis: {
                    ...
                    type: 'line', smooth: config.smooth || false, data: '{{binding:data1}}', name: '系列1'
                }]
            };
        ...
        case 'scatter':
            return {
                ...baseOption, xAxis: {
                    type: 'value', name: config.xAxisName
                }, yAxis: {
                    type: 'value', name: config.yAxisName
                }, series: [{
                    type: 'scatter', data: '{{binding:scatterData}}'
                }]
            };
        ...
        case 'area':
            return {
                ...baseOption, xAxis: {
                    ...
                    type: 'line', areaStyle: {}, data: '{{binding:data1}}', name: '系列1'
                }]
            };
...
function TemplateEditor({chart, onBack, onSaved}) {
    ...
    const [formData, setFormData] = useState({
        id: initialId, title: chart?.title || '新图表', chartType: chart?.chartType || 'bar', bindings: [{
            name: 'ds1',
            datasourceId: 'mock-adapter',
            query: 'mock:sales',
            mappingPath: '$.items[*].date',
            bindingKey: 'categories',
            stream: false
        }, {
            name: 'ds2',
            datasourceId: 'mock-adapter',
            query: 'mock:sales',
            mappingPath: '$.items[*].value',
            bindingKey: 'data1',
            stream: false
        }]
    });
      ...
      chartConfig(id: $id) { 
        ...
        try {
            ...
            if (json.data?.chartConfig) {
                ...
                setFormData({
                    id: cfg.id, title: cfg.title, chartType: cfg.chartType, bindings: cfg.bindings || []
                });
                // 从optionTemplate提取配置
                ...
                if (formData.chartType === 'pie') {
                    ...
                    setFormData(prev => ({
                        ...prev, bindings: [{
                            name: 'ds1',
                            datasourceId: 'mock-adapter',
                            query: 'mock:correlation',
                            mappingPath: '$.items[*]',
                            bindingKey: 'scatterData',
                            stream: false
                        }]
                    }));
                } else {
    ...
    return (<div className="template-editor">
            ...
            <div className="editor-left">
                <div className="editor-form">
                    ...
                    <div className="form-section">
                        ...
                        {/* 折线图特有配置 */}
                        {formData.chartType === 'line' && (<div className="form-field">
                        ...
                        {/* 饼图特有配置 */}
                        {formData.chartType === 'pie' && (<div className="form-field">
                            <label className="checkbox-label">
                                <input
                                    type="checkbox"
                    ...
                    <div className="form-section">
                        ...
                        {formData.bindings.map((binding, index) => (<div key={index} className="binding-card">
                            <div className="binding-card-header">
                                <span className="binding-num">#{index + 1}</span>
                                <span className="binding-key-tag">{binding.bindingKey}</span>
                                <button className="btn-remove-small" onClick={() => removeBinding(index)}>×
                                </button>
                            </div>
                            ...
                            <div className="binding-card-body">
                                ...
                                {/* Mock 查询 */}
                                {binding.datasourceId === 'mock-adapter' && (<div className="form-field">
                                    <label>Mock 数据集</label>
                                    <select
                                        value={binding.query}
                                        onChange={e => updateBinding(index, 'query', e.target.value)}
                                    >
                                        {MOCK_QUERIES.map(q => (<option key={q.id} value={q.id}>{q.name}</option>))}
                                    </select>
                                </div>)}

                                {/* REST API */}
                                {binding.datasourceId === 'rest-adapter' && (<div className="form-field">
                                    <label>API 地址</label>
                                    <input
                                        type="text"
                                        value={binding.query}
                                ...
                                {/* 数据库配置 */}
                                {binding.datasourceId === 'database-adapter' && (<DatabaseConfig
                                    binding={binding}
                                ...
                                <div className="form-field">
                                    ...
                                    <div className="jsonpath-chips">
                                        {JSONPATH_TEMPLATES.map((tpl, idx) => (<button
                                            key={idx}
                                            className="chip"
                        ...
                        {formData.bindings.length === 0 && (<div className="empty-state-small">
                            <p>暂无数据绑定</p>
        ...
        {message && (<div className={`editor-message ${message.type}`}>
            {message.text}
        </div>)}
    </div>);
}
...
function PreviewChart({formData, chartConfig}) {
    ...
    return (<div className="preview-container">
        {needsRefresh && (<div className="preview-refresh-hint">
            <span>⚠️ 配置已更改</span>
...
function DatabaseConfig({binding, onUpdate}) {
    ...
    return (<div className="database-config">
        {/* 数据库连接选择 */}
        <div className="form-field">
            <label>数据库连接</label>
            <select
            ...
            >
                ...
                {connections.map(conn => (<option key={conn.id} value={conn.id}>
        ...
        {/* 表选择 */}
        {binding.dbConnectionId && (<div className="form-field">
            <label>选择表</label>
            <select
        ...
        {/* 字段选择 */}
        {binding.dbTable && columns.length > 0 && (<div className="form-field">
            <label>选择字段（可多选）</label>
            <div className="column-checkboxes">
                {columns.map(col => (<label key={col.name} className="checkbox-label">
                    <input
                        type="checkbox"
        ...
        {/* 预览按钮 */}
        {binding.dbTable && (<div className="form-field">
        ...
        {/* 数据预览 */}
        {previewData.length > 0 && (<div className="data-preview">
            <table>
                <thead>
                <tr>
                ...
                {previewData.map((row, idx) => (<tr key={idx}>
                    {Object.values(row).map((val, i) => (<td key={i}>{val !== null ? String(val) : 'NULL'}</td>))}
                </tr>))}
                </tbody>
            </table>
        </div>)}
        ...
        {binding.dbUseCustomSql && (<div className="form-field">
        ...
        {/* 显示最终查询 */}
        {binding.dbConnectionId && binding.dbTable && (<div className="query-display">
            <label>最终查询</label>
